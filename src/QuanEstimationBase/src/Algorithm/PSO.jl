    ...
    for ei = 1:(max_episode[1]-1)
        ...
        for pk = 1:p_num
            ...
            for dk = 1:dim
                ...
                velocity[dk, pk] =
                    c0 * velocity[dk, pk] +
                    c1 * rand(opt.rng) * (pbest[dk, pk] - particles[pk].data.ψ0[dk]) +
                    c2 * rand(opt.rng) * (gbest[dk] - particles[pk].data.ψ0[dk])
        ...
        if ei % max_episode[2] == 0
            dynamics.data.ψ0 = [gbest[i] for i = 1:dim]
            particles = repeat(dynamics, p_num)
        end
        set_f!(output, fit_out)
        set_buffer!(output, gbest)
        ...
        for pk = 1:p_num
            ...
            for dk = 1:dim
                ...
                velocity_state[dk, pk] =
                    ...
                    c1 * rand(opt.rng) * (pbest_state[dk, pk] - particles[pk].data.ψ0[dk]) +
            ...
            for dk = 1:ctrl_num
                for ck = 1:ctrl_length
                    ...
                    velocity_ctrl[dk, ck, pk] =
                        ...
                        c1 *
                        rand(opt.rng) *
                        (pbest_ctrl[dk, ck, pk] - particles[pk].data.ctrl[dk][ck])
                    +c2 *
                    rand(opt.rng) *
                    (gbest_ctrl[dk, ck] - particles[pk].data.ctrl[dk][ck])
