function update!(opt::StateOpt, alg::NM, obj, dynamics, output)
    ...
    for pj in eachindex(ini_state)
        nelder_mead[pj].data.ψ0 = [ini_state[pj][i] for i = 1:dim]
    end
    for pj = (length(ini_state)+1):p_num
        r_ini = 2 * rand(opt.rng, dim) - ones(dim)
        r = r_ini / norm(r_ini)
        phi = 2 * pi * rand(opt.rng, dim)
        nelder_mead[pj].data.ψ0 = [r[i] * exp(1.0im * phi[i]) for i = 1:dim]
    end
    ...
    for ei = 1:(max_episode-1)
        ...
        for ni = 1:dim
            vec_ave[ni] = [nelder_mead[pk].data.ψ0[ni] for pk = 1:(p_num-1)] |> sum
        ...
        for nj = 1:dim
            vec_ref[nj] =
                vec_ave[nj] + ar * (vec_ave[nj] - nelder_mead[sort_ind[end]].data.ψ0[nj])
        end
        ...
        elseif fr < p_fit[sort_ind[end-1]]
            ...
            if fr <= p_fit[sort_ind[end]]
                ...
                for nl = 1:dim
                    vec_ic[nl] =
                        vec_ave[nl] -
                        ac * (vec_ave[nl] - nelder_mead[sort_ind[end]].data.ψ0[nl])
                end
                ...
                if fic > p_fit[sort_ind[end]]
                    ...
                    # shrink
                    vec_first = [nelder_mead[sort_ind[1]].data.ψ0[i] for i = 1:dim]
                    for pk = 1:p_num
                        for nq = 1:dim
                            nelder_mead[pk].data.ψ0[nq] =
                                vec_first[nq] +
                                as0 * (nelder_mead[pk].data.ψ0[nq] - vec_first[nq])
                        end
                        nelder_mead[pk].data.ψ0 =
                            nelder_mead[pk].data.ψ0 / norm(nelder_mead[pk].data.ψ0)
